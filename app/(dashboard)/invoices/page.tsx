  ...
  function applyImport() {
    ...
    if (importMode === "epic") {
      ...
      for (const t of chosen) {
        ...
        for (const te of (t.timeEntries ?? []) as Array<{ description: string | null; duration: number; startedAt: string }>) {
      ...
      newItems = chosen.map((t) => {
        const entries = (t.timeEntries ?? []) as Array<{ description: string | null; duration: number; startedAt: string }>;
        const descs = entries
      ...
      <div className="rounded-lg border overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="bg-card">
            {/* Table header */}
            <div className="grid grid-cols-[2fr_1.5fr_1.5fr_1fr_1fr_auto] gap-4 border-b bg-muted/30 px-4 py-2.5 text-caption font-semibold uppercase tracking-wider text-muted-foreground">
              <span>Rechnung</span>
              <span>Kunde</span>
              <span>Projekt</span>
              <span>Betrag</span>
              ...
              <div className="space-y-2">
                {formItems.map((item, i) => (
                  <div key={i} className="grid grid-cols-[1fr_80px_110px_90px_auto] gap-2 items-end">
                    ...
                    <div>
                      {i === 0 && <Label className="text-caption text-muted-foreground mb-1 block">Einzel­preis</Label>}
