      ...
      >
        <div className="p-3.5">
          <div className="space-y-2.5">
            {/* Epic pill (own row, only if epic exists) */}
            {task.epic && (
            ...
            {/* Title + icons (icons move into title row when no epic) */}
            <div className="flex items-start gap-2">
              ...
              {!task.epic && (task.approvalStatus === "PENDING" || recurrence || (task.clientVisible && !isClient)) && (
                <div className="flex items-center gap-0.5 shrink-0 -mr-1">
                  {task.approvalStatus === "PENDING" && (
                    <span className="flex h-5 w-5 items-center justify-center rounded text-warning" title="Wartet auf Abnahme">
                      <ClipboardCheck className="h-3 w-3" />
