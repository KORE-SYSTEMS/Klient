...
interface Proposal {
  ...
  client: { id: string; name: string | null; email: string; company: string | null } | null;
}
...
export default function PublicProposalPage() {
  ...
  if (notFound || !proposal || !workspace) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-gray-50 text-center px-4">
        ...
        <h1 className="text-xl font-bold text-gray-800">Angebot nicht gefunden</h1>
        <p className="text-sm text-gray-500 max-w-xs">
          Dieses Angebot ist nicht verfügbar oder der Link ist ungültig.
        </p>
      </div>
    );
  }
  ...
  const isExpired  = proposal.status === "EXPIRED" ||
    (proposal.validUntil && new Date(proposal.validUntil) < new Date() && proposal.status === "SENT");
    ...
    <>
      <style>{`
        @media print {
          ...
          .proposal-card { box-shadow: none !important; border: none !important; }
        }
      `}</style>
      ...
      <div className="min-h-screen bg-gray-50 py-10 px-4" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
        ...
        {/* Proposal card */}
        <div className="proposal-card max-w-3xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
          {/* Header */}
          <div style={{ backgroundColor: primaryColor }} className="px-10 py-8">
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                <p className="text-white/70 text-xs uppercase tracking-wider font-semibold">Angebot</p>
                <p className="text-white text-xl font-bold">{proposal.number}</p>
                <span
          ...
          {/* Accepted / Expired banner */}
          {isAccepted && (
            <div className="px-10 py-3 bg-emerald-50 border-b border-emerald-200">
              <p className="text-sm font-semibold text-emerald-700 text-center">
                Dieses Angebot wurde akzeptiert.
              </p>
            </div>
          )}
          {isExpired && !isAccepted && (
            <div className="px-10 py-3 bg-orange-50 border-b border-orange-200">
              <p className="text-sm font-semibold text-orange-700 text-center">
                Die Gültigkeitsdauer dieses Angebots ist abgelaufen.
              </p>
            </div>
          )}
          ...
          <div className="px-10 py-5 border-b border-gray-100 grid grid-cols-3 gap-6 bg-gray-50/50">
            <div>
              ...
              <p className="text-sm font-semibold text-gray-800">{fmtDate(proposal.issuedAt)}</p>
            </div>
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-1">Gültig bis</p>
              <p className={`text-sm font-semibold ${isExpired && !isAccepted ? "text-orange-600" : "text-gray-800"}`}>
                {fmtDate(proposal.validUntil)}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-1">Projekt</p>
              <p className="text-sm font-semibold text-gray-800">{proposal.project?.name ?? "—"}</p>
            </div>
          </div>
          ...
          <div className="px-10 py-5 border-b border-gray-100 grid grid-cols-2 gap-8">
            ...
            {proposal.client && (
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-2">An</p>
                <p className="text-sm font-semibold text-gray-800">{proposal.client.name ?? proposal.client.email}</p>
                {proposal.client.company && (
                  <p className="text-sm text-gray-500">{proposal.client.company}</p>
                )}
          ...
          <div className="px-10 pb-8">
            <div className="ml-auto max-w-xs rounded-xl p-5" style={{ backgroundColor: "#f9fafb" }}>
              ...
              <div className="flex justify-between text-sm text-gray-600 mb-3 pb-3 border-b border-gray-200">
                <span>MwSt. {proposal.taxRate}%</span>
